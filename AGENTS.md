...
- Web app: `pnpm -F @slice/web dev` (preview: `pnpm -F @slice/web start`, build: `pnpm -F @slice/web build`).
- API: `pnpm -F @slice/api dev` (typecheck: `pnpm -F @slice/api typecheck`).
...
- Imports: Use workspace packages (`@slice/*`) and web alias `@/*` to `apps/web/src`.
